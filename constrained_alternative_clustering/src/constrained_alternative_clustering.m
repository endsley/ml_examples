...
function [assignment_1, assignment_2, center_1, center_2] = constrained_alternative_clustering(k, k2, X)
	...
	[assignment_1 , center_1, sumd] = pack_kmeans(X,k, kmeans_repeat);
...
%	Generate Distance matrix and transform the data
	...
	[U,S,V] = svd(A);
	y = X*U*sqrt(S);
	X_alternative = X*U*inv(sqrt(S));


	[assignment_2 , center_2, sumd] = pack_kmeans(X_alternative,k2, kmeans_repeat);
end
