...
class DCN:
	...
	def compute_Linear_kernel(self, input_data):
	...
	def compute_Gaussian_kernel(self, input_data):				# Using random fourier features
	...
	def forward_pass(self, phi):		#	Gaussian Version
		...
		L = self.compute_Linear_kernel(Y)
		#L = self.compute_Gaussian_kernel(Y)
	...
	def update_W(self, U):
		...
		for lmda_count in range(1):
			while True:
				...
				if grad_norm < 0.0001: break
				if (np.absolute(new_cost.data.numpy() - cost.data.numpy()))/np.absolute(new_cost.data.numpy()) < 0.00001: break;
				if learning_rate < 0.00000001: break
	...
	def run(self):
		L = self.compute_Gaussian_kernel(self.xTor)
