...
def obtain_Wolfe_alpha(db, y_tilde, w_l, previous_gw, i_values, j_values, m):
	wolfe_passed = False
	alpha = 1
	beta = 0.5
	c1 = 0.0001
	...
	[df_before, cost_before] = Stochastic_W_gradient(db, y_tilde, previous_gw, w_l, i_values, j_values)
	#df_before = get_orthogonal_vector(db, m+1, df_before) # m+1 is to also remove the current dimension

	while not wolfe_passed:	
		alpha = np.power(beta,itr)
		if m == 0:
			new_W = w_l - 0.1*df_before
		else:
			new_W = np.sqrt(1-alpha*alpha)*w_l - alpha*df_before

		[df_after, cost_after] = Stochastic_W_gradient(db, y_tilde, previous_gw, new_W, i_values, j_values)
	
		print cost_before, cost_after
		import pdb; pdb.set_trace()
		if (cost_before - cost_after) >= c1*alpha*df_before.dot(df_before):		# Armijo rule based on minimization
			wolfe_passed = True
		else:
			itr = itr+1
...
#	Note :  This is based on Minimizing approach
...
def W_optimize_Gaussian(db):
	...
	for m in range(db['q']):
		...
		while not w_converged:
			...
			wolfe_alpha = obtain_Wolfe_alpha(db, y_tilde, w_l, previous_gw, i_values, j_values,m)
